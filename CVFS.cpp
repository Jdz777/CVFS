...
int CreateFile(char *name, int permission)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode->Buuffer = (char *)malloc(MAXFILESIZE);
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    if (((UFDTArr[fd].ptrfiletable->ptrinode->permisssion) != WRITE) && ((UFDTArr[fd].ptrfiletable->ptrinode->permission) != READ + WRITE))
        return -1;
    ...
    (UFDTArr[fd].ptrfiletable->ptrinode->ActualFileSize) = (UFDTAr[fd].ptrfiletable->ptrinode->FileActualSize) + isize;
    return isize;
}
    ...
    if ((UFDTArr[fd].ptrfiletable->mode == READ) || (UFDTArr[fd].ptrfiletable->mode == READ + WRITE))
    {
        if (from == CURRENT)
        {
            ...
            if (((UFDTArr[fd].ptrfiletable->readoffset) + size) < 0)
                return -1;
            (UFDTArr[fd].ptrfiletable->reaoffset) = (UFDTArr[fd].ptrfiletable->readoffset) + size;
        }
        else if (from == START)
        {
